...
class _SessionDetailsScreenState extends State<SessionDetailsScreen> {
  ...
  Future<void> _loadCreatorProfile() async {
    try {
      ...
      final profile = await appState.firebaseService.getUserProfileAsync(widget.session.creatorId);
      if (mounted) {
        setState(() {
  ...
  Future<void> _loadApplicationsData() async {
    ...
    try {
      ...
      if (isCreator) {
        ...
        _allApplications = await appState.firebaseService.getCollabSessionApplicationsAsync(widget.session.id ?? '');
        // Fetch profiles of applicants in parallel
        for (var app in _allApplications) {
          if (!_loadedProfiles.containsKey(app.userId)) {
            final profile = await appState.firebaseService.getUserProfileAsync(app.userId);
            if (profile != null) {
        ...
        _myApplication = await appState.firebaseService.getCollabSessionApplicationAsync(widget.session.id ?? '', userId);
      }
    } catch (e) {
  ...
  Future<void> _requestToJoin() async {
    ...
    try {
      ...
      await appState.firebaseService.applyToCollabSessionAsync(widget.session.id ?? '', userId, newApp);
      await _loadApplicationsData();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Request to join sent!'), backgroundColor: AppTheme.success),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to submit request: $e'), backgroundColor: AppTheme.danger),
        );
      }
    } finally {
  ...
  Future<void> _cancelRequest() async {
    ...
    setState(() => _isActionInProgress = true);
    try {
      await appState.firebaseService.cancelCollabSessionApplicationAsync(widget.session.id ?? '', userId);
      await _loadApplicationsData();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Join request cancelled.'), backgroundColor: AppTheme.warning),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _updateApplicationStatus(String applicantId, String newStatus) async {
    ...
    setState(() => _isActionInProgress = true);
    try {
      await appState.firebaseService.updateCollabApplicationStatusAsync(widget.session.id ?? '', applicantId, newStatus);
      await _loadApplicationsData();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Request marked as $newStatus'), backgroundColor: AppTheme.success),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to update request: $e'), backgroundColor: AppTheme.danger),
        );
      }
    } finally {
      if (mounted) setState(() => _isActionInProgress = false);
  ...
  Future<void> _deleteSession() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete this collab session permanently?',
    ...
    if (confirm == true) {
      ...
      try {
        final appState = Provider.of<AppState>(context, listen: false);
        await appState.firebaseService.deleteCollabSessionAsync(widget.session.id ?? '');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Session deleted successfully'), backgroundColor: AppTheme.success),
          );
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to delete session: $e'), backgroundColor: AppTheme.danger),
          );
        }
      } finally {
        if (mounted) setState(() => _isActionInProgress = false);
  ...
  Future<void> _closeSession() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Session closed successfully'), backgroundColor: AppTheme.success),
        );
        Navigator.pop(context);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to close session: $e'), backgroundColor: AppTheme.danger),
        );
      }
    } finally {
      if (mounted) setState(() => _isActionInProgress = false);
  ...
  Widget build(BuildContext context) {
    ...
    final dateStr = widget.session.isDateFlexible
        ? 'Flexible Date'
        : (widget.session.startDateTime != null ? widget.session.startDateTime!.substring(0, 16).replaceAll('T', ' ') : 'Date not set');
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  if (isCreator) ...[
                    IconButton(
                      ...
                      onPressed: () async {
                        final updated = await Navigator.pushNamed(context, '/create-session', arguments: widget.session);
                        if (updated == true && mounted) {
                          Navigator.pop(context); // pop detail view to reload parent lists
                        }
                      },
                    ),
                    IconButton(
                      icon: const Icon(Icons.close_rounded, color: AppTheme.warning),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        const Icon(Icons.calendar_today_outlined, color: AppTheme.primaryAccent, size: 16),
                    ...
                    Row(
                      children: [
                        const Icon(Icons.location_on_outlined, color: AppTheme.primaryAccent, size: 16),
                        const SizedBox(width: 12),
                        Text(
                ...
                children: widget.session.lookingForRoles.map((role) {
                  return Container(
                    ...
                    child: Text(
                      ...
                      style: GoogleFonts.inter(fontSize: 12, color: AppTheme.secondaryAccent, fontWeight: FontWeight.bold),
                    ),
                  );
                }).toList(),
              ),
              ...
              if (_creatorProfile != null) ...[
                ...
                InkWell(
                  onTap: () {
                    Navigator.pushNamed(context, '/profile-detail', arguments: _creatorProfile!);
                  },
                  borderRadius: BorderRadius.circular(16),
                  child: Container(
                    padding: const EdgeInsets.all(16),
                    ...
                    child: Row(
                      children: [
                        CircleAvatar(
                          ...
                          backgroundImage: _creatorProfile!.profilePictureUrl != null && _creatorProfile!.profilePictureUrl!.isNotEmpty
                              ? NetworkImage(_creatorProfile!.profilePictureUrl!)
                              : null,
                        ...
                        if (!isCreator)
                          IconButton(
                            icon: const Icon(Icons.chat_bubble_outline_rounded, color: AppTheme.primaryAccent),
                    ...
                    return Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          if (profile != null)
                            IconButton(
                              icon: const Icon(Icons.chat_bubble_outline_rounded, color: AppTheme.primaryAccent, size: 16),
                              ...
                              return Container(
                                ...
                                child: Row(
                                  children: [
                                          ...
                                          Text(
                                            ...
                                            style: GoogleFonts.inter(
                                                ...
                                                color: app.status == 'accepted'
                                                    ? AppTheme.success
                                                    : (app.status == 'declined' ? AppTheme.danger : AppTheme.warning)),
                                          ),
                                        ],
                                      ),
                                    ),
                                    if (profile != null)
                                      IconButton(
                                        icon: const Icon(Icons.chat_bubble_outline_rounded, color: AppTheme.primaryAccent, size: 18),
                                        onPressed: () => _openDirectChat(profile),
                                      ),
                                    if (app.status == 'pending') ...[
                                      IconButton(
                                        icon: const Icon(Icons.check_circle_outline_rounded, color: AppTheme.success, size: 20),
                                        onPressed: _isActionInProgress ? null : () => _updateApplicationStatus(app.userId, 'accepted'),
                                      ),
                                      IconButton(
                                        icon: const Icon(Icons.cancel_outlined, color: AppTheme.danger, size: 20),
                                        onPressed: _isActionInProgress ? null : () => _updateApplicationStatus(app.userId, 'declined'),
                                      ),
                                    ],
                                  ],
                                ),
                                    ...
                                    Text(
                                      ...
                                      style: GoogleFonts.inter(
                                        ...
                                        color: _myApplication!.status == 'accepted'
                                            ? AppTheme.success
                                            : (_myApplication!.status == 'declined' ? AppTheme.danger : AppTheme.warning),
                                      ),
                                    ),
                                    if (_myApplication!.status == 'pending')
                                      TextButton.icon(
                                        onPressed: _isActionInProgress ? null : _cancelRequest,
                                        icon: const Icon(Icons.remove_circle_outline_rounded, color: AppTheme.danger, size: 16),
                                        label: Text(
                                          'Cancel Request',
                                          style: GoogleFonts.inter(color: AppTheme.danger, fontSize: 13, fontWeight: FontWeight.bold),
                                        ),
                                      ),
                                  ],
                                ),
