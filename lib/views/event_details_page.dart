...
class _EventDetailsPageState extends State<EventDetailsPage> {
  ...
  Future<void> _loadMembersAndProfiles() async {
    ...
    try {
      ...
      if (_event != null) {
        final allEvents = await appState.firebaseService.getBandEventsListAsync(
          widget.bandId,
        );
        ...
        if (_event!.parentEventId != null &&
            _event!.parentEventId!.isNotEmpty) {
          _linkedSubEvents = allEvents
              .where((e) => e.parentEventId == _event!.parentEventId)
              .toList();
        } else {
          final normTarget = normalizeTitle(_event!.title);
          _linkedSubEvents = allEvents
              .where((e) => normalizeTitle(e.title) == normTarget)
              .toList();
        }
  ...
  Future<void> _triggerReminder(String reminderType) async {
    if (_isSendingReminder) return; // Prevent double taps
    if (!_isAdmin) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Only Leaders or Admins can trigger event reminders."),
    ...
    try {
      final res = await appState.firebaseService.triggerEventReminderAsync(
        widget.bandId,
        widget.eventId,
        reminderType,
      );
      ...
      final successCount = res['successCount'] ?? 0;
      final failureCount = res['failureCount'] ?? 0;
      ...
      switch (status) {
        case 'completed':
          message =
              "🔔 Sent push notifications to $successCount pending member(s)!";
          break;
        case 'partial_success':
          message =
              "⚠️ Sent push notifications to $successCount member(s) ($failureCount failed).";
          bgColor = Colors.orange;
          break;
        case 'no_non_responders':
          message = "ℹ️ All band members have already responded to this event!";
          bgColor = AppTheme.secondaryAccent;
          break;
        case 'no_valid_tokens':
          message =
              "⚠️ Non-responding members have no registered push notification tokens.";
          bgColor = Colors.orange;
          break;
        case 'already_sent':
          message =
              "ℹ️ A $reminderType reminder has already been sent for this event.";
          bgColor = AppTheme.secondaryAccent;
          break;
        case 'error':
          ...
          if (code == 'permission-denied') {
            message =
                "⛔ Permission Denied: Only band Leaders or Admins can trigger reminders.";
          } else {
        ...
        default:
          message =
              "Notification result: ${status ?? 'done'} ($successCount succeeded)";
      }
  ...
  Widget _buildGodModeButton({
    ...
    final isDisabled = _isSendingReminder;
    return AnimatedTapDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isDisabled ? Colors.grey : color.withOpacity(0.6),
            width: 1,
          ),
        ),
        child: Row(
  ...
  Widget _getEventIcon(String type) {
    ...
    return Container(
      ...
      child: Text(emoji, style: GoogleFonts.inter(fontSize: 28)),
    );
  }
  ...
  void _respond(String status, String comment) async {
    ...
    if (_event!.isLocked) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "This event is locked and responses can no longer be changed.",
          ),
          backgroundColor: AppTheme.danger,
        ),
      );
  ...
  void _deleteEvent() async {
    ...
    if (confirmed == true && mounted) {
      final appState = Provider.of<AppState>(context, listen: false);
      try {
        await appState.firebaseService.deleteBandEventAsync(
          widget.bandId,
          widget.eventId,
        );
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    for (var member in _members) {
      ...
      final name =
          _cachedProfiles[userId]?.displayName ??
          _cachedProfiles[userId]?.nickname ??
          member.nickname ??
          'Unknown Member';
      final response = event.responses[userId]?.status;
      final comment =
          event.responses[userId]?.uncertainReason ??
          event.responses[userId]?.comment;
      final displayName = comment != null && comment.isNotEmpty
          ? '$name\n"$comment"'
          : name;
    ...
    return GradientScaffold(
      appBar: CustomTopBar(title: event.title, showBack: true),
      body: SafeArea(
        child: ListView(
          ...
          children: [
                                ...
                                SingleChildScrollView(
                                  ...
                                  child: Row(
                                    children: _linkedSubEvents.map((sub) {
                                      ...
                                      final seqStr =
                                          sub.subEventSequence != null
                                          ? 'Part ${sub.subEventSequence}'
                                          : sub.title;
                ...
                builder: (context) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      if (_localSelectedResponse == 'UNCERTAIN' ||
                          _localSelectedResponse == 'maybe') ...[
            ...
            if (event.description.isNotEmpty ||
                event.additionalNotes.isNotEmpty) ...[
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  color: AppTheme.cardBackground,
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _buildResponseGroup(
                      'Unanswered (${noResponseList.length})',
                      noResponseList,
                      AppTheme.textSecondary,
                    ),
                  ],
              ...
              if (event.externalInvitees.isNotEmpty) ...[
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _buildResponseGroup(
                        'Guest YES (${extYesList.length})',
                        extYesList,
                        AppTheme.success,
                      ),
                      _buildResponseGroup(
                        'Guest UNCERTAIN (${extUncertainList.length})',
                        extUncertainList,
                        AppTheme.warning,
                      ),
                      _buildResponseGroup(
                        'Guest NO (${extNoList.length})',
                        extNoList,
                        AppTheme.danger,
                      ),
                      _buildResponseGroup(
                        'Guest Pending (${extPendingList.length})',
                        extPendingList,
                        Colors.grey,
                      ),
                    ],
                  ...
                  onTap: () async {
                    final confirmed = await showDialog<bool>(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: Text(
                          "Are you sure all subs and members are confirmed and everything is a go? Finalizing locks RSVPs and sets the event status to Finalized.",
                          style: GoogleFonts.inter(
                            color: AppTheme.textSecondary,
                          ),
                        ),
                    ...
                    if (confirmed == true && mounted) {
                      try {
                        await appState.firebaseService.lockBandEventAsync(
                          widget.bandId,
                          widget.eventId,
                        );
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                "Event finalized successfully! All subs/members confirmed.",
                              ),
                              backgroundColor: AppTheme.success,
                            ),
                          );
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text("Failed to finalize event: $e"),
                              backgroundColor: AppTheme.danger,
                            ),
                          );
                        }
                  ...
                  onTap: () async {
                    final confirmed = await showDialog<bool>(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: Text(
                          "Do you want to unlock this event and allow members/subs to modify their RSVP status again?",
                          style: GoogleFonts.inter(
                            color: AppTheme.textSecondary,
                          ),
                        ),
                    ...
                    if (confirmed == true && mounted) {
                      try {
                        await appState.firebaseService.unlockBandEventAsync(
                          widget.bandId,
                          widget.eventId,
                        );
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text("Event unlocked. RSVPs re-opened."),
                              backgroundColor: AppTheme.success,
                            ),
                          );
                        }
                        ...
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text("Failed to unlock event: $e"),
                              backgroundColor: AppTheme.danger,
                            ),
                          );
                        }
